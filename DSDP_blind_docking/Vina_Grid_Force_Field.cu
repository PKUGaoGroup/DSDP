...
void VINA_GRID_FORCE_FIELD::GRID::Cuda_Texture_Initial(VINA_GRID_FORCE_FIELD* vgff)
{
	...
	for (int i = 0; i < vina_grid_force_field->type_numbers; i = i + 1)
	{
		...
		float4* temp_ptr = &potential[(size_t)i * vina_grid_force_field->grid_numbers];
...
static __global__ void Add_Sphere_Force_Field_Device
(
	const int grid_numbers, const INT_VECTOR grid_dimension, const int type_numbers, float4* potential,
	const VECTOR grid_min, const VECTOR grid_length,
	const int point_numbers,const VECTOR *point_crd,const float sphere_radius, const float sphere_border_strenth,
	const VECTOR move_vec
)
{
	INT_VECTOR grid_3d_serial;
	int layer_numbers = grid_dimension.int_x * grid_dimension.int_y;
	...
	for (int grid_i = blockIdx.x * blockDim.x + threadIdx.x; grid_i < grid_numbers; grid_i = grid_i + gridDim.x * blockDim.x)
	{
		...
		dr_abs_min = 100000.f;//a very big number
		for (int point_i = 0; point_i < point_numbers; point_i += 1)
		{
			point_i_crd = point_crd[point_i];
			point_i_crd.x += move_vec.x;
			point_i_crd.y += move_vec.y;
			point_i_crd.z += move_vec.z;

			dr.x = point_i_crd.x - grid_crd.x;
			dr.y = point_i_crd.y - grid_crd.y;
			dr.z = point_i_crd.z - grid_crd.z;
			dr_abs = norm3df(dr.x, dr.y, dr.z);
			if (dr_abs < sphere_radius)
			{
				is_out_of_border = false;
				break;
			}//Èç¹ûËùŽŠžñµãÔÚÄ³žöpointµÄÇò¿ÇÄÚ²¿Ôò²»ÊÜµœ±ßœç×÷ÓÃ
			if (dr_abs < dr_abs_min)
			{
				dr_abs_min = dr_abs;
				dr_min = dr;
			}//ŒÇÂŒÈç¹ûÔÚËùÓÐpointÇò¿ÇÖ®Íâ£¬ÄÇÃŽÀëÆä×îœüµÄÇò¿ÇpointµÄÊžÁ¿ÓÃÓÚŒÓÁŠ
		}
		if (is_out_of_border == true)
		{
			float surface_dr_abs = dr_abs_min - sphere_radius;//Ó°ÏìÁŠµÄÊµŒÊÇ¿¶È
			float energy = 0.5f * sphere_border_strenth * surface_dr_abs * surface_dr_abs;
			float frc_abs = sphere_border_strenth * surface_dr_abs / dr_abs_min;
			VECTOR frc = { dr_min.x * frc_abs,dr_min.y * frc_abs ,dr_min.z * frc_abs };
			for (int type_i = 0; type_i < type_numbers; type_i = type_i + 1)
			{
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].w, energy);
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].x, frc.x);//·ûºÅÊÇÒòÎªÇ°ÃæµÄdrÊžÁ¿ÊÇµ°°×Ô­×Ó×ø±êŒõÈ¥žñµã×ø±ê
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].y, frc.y);
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].z, frc.z);
			}
		}
	}
}
void VINA_GRID_FORCE_FIELD::GRID::Add_Sphere_Force_Field(const int point_numbers, const VECTOR* point_crd, const VECTOR move_vec, const float sphere_radius,const float sphere_border_strenth)
{
	if (malloced_point_numbers >= point_numbers)
	{
		;
	}
	else
	{
		if (d_point_crd != NULL)
		{
			cudaFree(d_point_crd);
		}
		cudaMalloc((void**)&d_point_crd, sizeof(VECTOR) * point_numbers);
		malloced_point_numbers = point_numbers;
	}
	cudaMemcpy(d_point_crd, point_crd, sizeof(VECTOR) * point_numbers, cudaMemcpyHostToDevice);

	Add_Sphere_Force_Field_Device << <64, 64 >> >
		(
			vina_grid_force_field->grid_numbers, vina_grid_force_field->grid_dimension, vina_grid_force_field->type_numbers, potential,
			vina_grid_force_field->grid_min, vina_grid_force_field->grid_length,
			point_numbers, d_point_crd, sphere_radius, sphere_border_strenth,
			move_vec
			);
	Copy_Potential_To_Texture();
}
