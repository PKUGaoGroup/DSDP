...
void VINA_GRID_FORCE_FIELD::GRID::Copy_Potential_To_Texture()
{
	for (int i = 0; i < vina_grid_force_field->type_numbers; i = i + 1)
	{
		cudaError_t test=cudaMemcpy3D(&copyParams_potential[i]);
	}
}
	...
	for (int grid_i = blockIdx.x * blockDim.x + threadIdx.x; grid_i < grid_numbers; grid_i = grid_i + gridDim.x * blockDim.x)
	{
		...
		for (int point_i = 0; point_i < point_numbers; point_i += 1)
		{
			...
			if (dr_abs < sphere_radius)
			{
				...
				break;
			}//如果所处格点在某个point的球壳内部则不受到边界作用
			if (dr_abs < dr_abs_min)
			{
				dr_abs_min = dr_abs;
				dr_min = dr;
			}//记录如果在所有point球壳之外，那么离其最近的球壳point的矢量用于加力
		}
		if (is_out_of_border == true)
		{
			float surface_dr_abs = dr_abs_min - sphere_radius;//影响力的实际强度
			...
			{
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].w, energy);
				atomicAdd(&potential[(size_t)type_i * grid_numbers + grid_i].x, frc.x);//符号是因为前面的dr矢量是蛋白原子坐标减去格点坐标
