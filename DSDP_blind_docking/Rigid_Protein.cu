...
void RIGID_PROTEIN::Initial_Protein_From_PDBQT(const char* file_name, const VECTOR box_length)
{
	...
	{
		protein_center.x += crd[i].x;
		protein_center.y += crd[i].y;
		protein_center.z += crd[i].z;
	}
	protein_center.x = (float)protein_center.x / atom_numbers;
	protein_center.y = (float)protein_center.y / atom_numbers;
	protein_center.z = (float)protein_center.z / atom_numbers;
