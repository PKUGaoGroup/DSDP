...
//below parameters can be changed while command line input, but default may be good
	...
	{
		if (site_info.site_point[i].x > box_max.x
			|| site_info.site_point[i].y > box_max.y
			|| site_info.site_point[i].z > box_max.z
			|| site_info.site_point[i].x < box_min.x
			|| site_info.site_point[i].y < box_min.y
			|| site_info.site_point[i].z < box_min.z)
		{
...
//	printf("time %lf s\n", time_start);
	...
	copy_pdbqt.Append_Frame_To_Opened_pdbqt_standard(out_pdbqt, &crd_record[(size_t)energy_record[0].id * (molecule[0].atom_numbers)], move_vec,0,energy_record[0].energy);
	fprintf(out_list, "%s %f\n", site_npy_name, energy_record[0].energy);
	//如果要去重排序输出前n个构象
	...
	{
		copy_pdbqt.Append_Frame_To_Opened_pdbqt_standard(out_pdbqt, &DSDP_sort.selected_crd[(size_t)i* (molecule[0].atom_numbers)], move_vec,i,energy_record[i].energy);
		fprintf(out_list, "%s %f\n", site_npy_name, energy_record[i].energy);	
	}	
